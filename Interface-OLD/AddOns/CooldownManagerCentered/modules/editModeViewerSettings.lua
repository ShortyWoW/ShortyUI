...
local function HidePanel()
    ...
    if dialog and LEM.HideAttachedSettingsPanel then
        LEM:HideAttachedSettingsPanel(dialog)
    end
end
...
local function ShowPanelFor(frameName)
    ...
    local panel = LEM:ShowAttachedSettingsPanel(dialog, {
        title = PANEL_TITLE,
        settings = settings,
        recreate = recreate,
        showReset = false,
    ...
    if panel then
        ...
        C_Timer.After(0, function()
            if LEM:IsAttachedSettingsPanelShown(dialog) then
                AnchorPanel(panel, dialog)
